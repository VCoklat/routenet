...
def train(args):
    ...
    file_train = glob.glob(args.train[0])
    file_eval = glob.glob(args.eval_[0])

    train_spec = tf.estimator.TrainSpec(input_fn=lambda:tfrecord_input_fn(file_train,hparams,shuffle_buf=args.shuffle_buf,target=args.target),max_steps=args.train_steps)
    eval_spec = tf.estimator.EvalSpec(input_fn=lambda:tfrecord_input_fn(file_eval,hparams,shuffle_buf=None,target=args.target),throttle_secs=10*60)
